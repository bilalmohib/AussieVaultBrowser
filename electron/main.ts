...
function createBrowserWindow(isMain: boolean = false): BrowserWindow {
  ...
  newWindow.webContents.setWindowOpenHandler((details) => {
    ...
    if (oauthProviders.some(provider => url.startsWith(provider))) {
      // Open OAuth URLs in a new BrowserWindow
      const authWindow = new BrowserWindow({
        ...
        webPreferences: {
          ...
          contextIsolation: true,
          webSecurity: true,
          partition: 'persist:shared-auth' // Use the same shared session
        }
      });
      ...
      authWindow.webContents.on('will-navigate', (event, newUrl) => {
        if (newUrl.startsWith('aussievault://callback')) {
          ...
          // Here you would parse the newUrl to extract the code/token
          // and then pass it back to the renderer process or handle it directly.
      ...
      authWindow.webContents.on('did-redirect-navigation', (event, newUrl) => {
        if (newUrl.startsWith('aussievault://callback')) {
          ...
          // Handle the redirect URL after a successful login
          authWindow.close();
          event.preventDefault();
        }
      });

      return { action: 'deny' }; // Deny the original window from opening it
    }
